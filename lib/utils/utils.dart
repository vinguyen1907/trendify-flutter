...
class Utils {
  ...
  static void showSnackBar({required BuildContext context, required String message}) {
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(message, style: Theme.of(context).textTheme.bodyMedium!.copyWith(color: Theme.of(context).colorScheme.onPrimaryContainer))));
  }

  static void showSnackBarSuccess({required BuildContext context, required String message, required String title, Widget? actionButton}) {
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
  ...
  static Future<void> showEnterPasscodeBottomSheet(
      {required BuildContext context, required String passcode, required VoidCallback onTruePasscode}) async {
    await showDialog(
        context: context,
        builder: (_) {
          return AlertDialog(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Pinput(
                    ...
                    errorTextStyle: AppStyles.labelMedium.copyWith(color: Theme.of(context).colorScheme.error, fontWeight: FontWeight.w500),
                    autofocus: true,
                    length: 6,
                    obscureText: true,
                    crossAxisAlignment: CrossAxisAlignment.center,
                      ...
                      if (snapshot.hasData) {
                        if (snapshot.data == true) {
                          return OutlinedButton(
                            ...
                            onPressed: () {
                              LocalAuthUtil.authenticateWithBiometrics().then((value) {
                                if (value) {
                                  onTruePasscode();
                                } else {
                                  Utils.showSnackBar(context: context, message: "Your device does not support");
                                }
                              });
                            },
                            child: Row(
                              ...
                              children: [
                                const MyIcon(icon: AppAssets.icFingerprint),
  ...
  static Future<void> showPayingDialog({required BuildContext context}) async {
    await showDialog(
              ...
              child: Column(
                children: [
                  Container(
                    height: 60,
                    width: 60,
                    decoration: BoxDecoration(color: AppColors.primaryColor, borderRadius: AppDimensions.circleCorners),
                    alignment: Alignment.center,
                    child: icon,
                  ),
                  const SizedBox(height: 20),
                  const Text("Successful!", textAlign: TextAlign.center, style: AppStyles.displayLarge),
                  const SizedBox(height: 20),
                  Text(
                    description,
  ...
  String getDescription(String apptransid) => "Ecommerce App thanh toán cho đơn hàng  #$apptransid";
